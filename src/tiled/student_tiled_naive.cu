...
*/
__global__ void kernel_naive_tiled(const uint8_t *const dev_inPtr, uint32_t *const dev_outPtr, const uint32_t inWidth, const uint32_t inHeight,
                                   const uint32_t outWidth, const uint32_t outHeight)
{
    ...
    {
        // Maximum entre le pixel courant et le pixel en sortie
...
float tiled_GPU_naive(los::Heightmap in, los::Heightmap &out)
{
    ...
    dim3 dimBlock(NB_THREAD_TILED_NAIVE, NB_THREAD_TILED_NAIVE); // Nombre de threads par block
    dim3 dimGrid(ceil((float)inWidth / (float)dimBlock.x), ceil((float)inHeight / (float)dimBlock.y));

    ChronoGPU chr;
    chr.start();
    // Kernel
    kernel_naive_tiled<<<dimGrid, dimBlock>>>(dev_inPtr, dev_tmpPtr, inWidth, inHeight, outWidth, outHeight);
    chr.stop();
